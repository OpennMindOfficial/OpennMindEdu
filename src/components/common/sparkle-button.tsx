...
import '@/styles/sparkle-button.css'; // Import the CSS styles
...
const SparkleButton = React.forwardRef<HTMLButtonElement, SparkleButtonProps>(
  ({ children, className, wrapperClassName, ...props }, ref) => {
    return (
      <div className={cn("sp sparkle-button-wrapper", wrapperClassName)}>
        <button ref={ref} className={cn("sparkle-button", className)} {...props}>
        ...
        <div className="bodydrop"></div> {/* This seems unused, but keeping it from original HTML */}
        <span aria-hidden="true" className="particle-pen">
          {/* Repeat particle SVGs 12 times as in the original HTML */}
          {[...Array(12)].map((_, i) => (
